        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          {skills.map((group, i) => (
            <motion.div
              ...
              className="hud-panel glass-panel-strong p-6 rounded-2xl group hover:border-primary/40 transition-all duration-300 border-white/5 hover:scale-[1.02]"
            >
              ...
              <div className="flex flex-wrap gap-2">
                {group.items.map(skill => (
                  <span
                    key={skill}
                    className="px-3 py-1 text-sm bg-black/40 border border-white/5 rounded-full text-muted-foreground group-hover:text-foreground transition-colors hover:border-primary/30 hover:bg-primary/5"
                  >
                    {skill}
                  </span>
                ))}
